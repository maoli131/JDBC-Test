...
The flights data comes from http://www.cs.washington.edu/education/courses/cse344/flights-small.tar.gz. 
...
- Method 2: the original way:
    ...
    0. Download the flights-small.tar.gz from the link provided above. Unzip and store the flighs-small.csv in
       your working directory.  

    1. Run first two lines of largeFlight.sql. By doing so, you create a empty table named flight-data.

    2. Import all data from the flights-small.csv to the table we created. 
    In terminal, run sqlite3 test.db; .mode csv; .import flights-small.csv flight_data

    3. Create our own flight table with only Airline, Origin, Destination. 
    To do so, run the rest 3 lines of largeFlight.sql.

    4. Run your PA2.java to get result and store it in "connection".

    5. To check your answer, load solution.csv into your database as a "solution" table. And check diff.

    6. Also you can output the Connected table to a csv file named your_solu.sql.
